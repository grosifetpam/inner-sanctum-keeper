        ...
        {isAdding && (
          <AdminFormCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <select value={form.fromAlterId} onChange={e => setForm({ ...form, fromAlterId: e.target.value })} className="bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground">
                {data.alters.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              </select>
              <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value as AlterRelation['type'] })} className="bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground">
                {relationTypes.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select value={form.toAlterId} onChange={e => setForm({ ...form, toAlterId: e.target.value })} className="bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground">
                {data.alters.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              </select>
            </div>

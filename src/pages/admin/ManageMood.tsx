...
export default function ManageMood() {
  ...
  const chartData = data.moods.slice(-14).map(m => ({ date: m.date, énergie: m.energyLevel, alter: getAlterName(m.alterId) }));
        ...
        {isAdding && (
          <AdminFormCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <input value={form.mood} onChange={e => setForm({ ...form, mood: e.target.value })} placeholder="Humeur" className="bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground" />
              <div>
                <label className="text-xs font-ui text-muted-foreground">Énergie: {form.energyLevel}/10</label>
                <input type="range" min="0" max="10" value={form.energyLevel} onChange={e => setForm({ ...form, energyLevel: parseInt(e.target.value) })} className="w-full mt-1 accent-primary" />
              </div>
            </div>
            <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Notes" className="w-full bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground mt-3 min-h-[60px]" />

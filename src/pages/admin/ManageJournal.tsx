...
import { Plus, Trash2, Edit2, X, Eye, EyeOff, Lock } from 'lucide-react';
...
export default function ManageJournal() {
  ...
  const startNew = () => {
    setEditing({
      ...
      alterId: data.alters[0]?.id || '', title: '', content: '', tags: [],
      isPublic: true, isPrivateAlterJournal: false,
    });
    setIsNew(true);
    setTagInput('');
  };
  ...
  const save = () => {
    ...
    if (isNew) addJournalEntry(editing);
    else updateJournalEntry(editing);
    setEditing(null);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-display text-foreground">Journal</h1>
        ...
        <div className="card-grimoire p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={editing.alterId} onChange={e => setEditing({ ...editing, alterId: e.target.value })}
                className="w-full bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground mt-1">
            ...
            <div className="flex items-center gap-4 mt-6">
              <button onClick={() => setEditing({ ...editing, isPublic: !editing.isPublic })}
                className={`flex items-center gap-1 text-xs font-ui ${editing.isPublic ? 'text-gold' : 'text-muted-foreground'}`}>
                {editing.isPublic ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                {editing.isPublic ? 'Public' : 'Privé'}
              </button>
              <button onClick={() => setEditing({ ...editing, isPrivateAlterJournal: !editing.isPrivateAlterJournal })}
                className={`flex items-center gap-1 text-xs font-ui ${editing.isPrivateAlterJournal ? 'text-primary' : 'text-muted-foreground'}`}>
                <Lock className="w-4 h-4" />
                {editing.isPrivateAlterJournal ? 'Journal privé' : 'Journal commun'}
              </button>
            </div>
          </div>
          <div className="mt-4">
            ...
            <textarea value={editing.content} onChange={e => setEditing({ ...editing, content: e.target.value })}
              className="w-full bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground mt-1 min-h-[120px]" />
          </div>
          <div className="mt-4">
            ...
            <div className="flex gap-2 mt-1">
              <input value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addTag())}
                className="flex-1 bg-input border border-border rounded px-3 py-2 text-sm font-ui text-foreground" placeholder="Ajouter un tag" />
      ...
      <div className="space-y-2">
        {data.journal.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(entry => (
          <div key={entry.id} className="card-grimoire p-4 flex items-center justify-between">
            <div>
